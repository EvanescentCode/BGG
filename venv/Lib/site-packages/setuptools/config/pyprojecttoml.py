...
def read_configuration(
    ...
    if setuptools_table:
        ...
        msg = "Support for `[tools.setuptools]` in `pyproject.toml` is still *beta*."
    ...
    # Persist changes:
    asdict["tools"] = tool_table
    tool_table["setuptools"] = setuptools_table

    try:
        # Don't complain about unrelated errors (e.g. tools not using the "tools" table)
        subset = {"project": project_table, "tools": {"setuptools": setuptools_table}}
...
def _skip_bad_config(
    ...
    if setuptools_cfg:
        # If `[tools.setuptools]` is set, then `pyproject.toml` config is intentional
        return False
...
class _ConfigExpander:
    def __init__(
        ...
        self.setuptools_cfg = config.get("tools", {}).get("setuptools", {})
    ...
    def _expand_all_dynamic(self, dist: "Distribution", package_dir: Mapping[str, str]):
        ...
        # `None` indicates there is nothing in `tools.setuptools.dynamic` but the value
    ...
    def _ensure_previously_set(self, dist: "Distribution", field: str):
        ...
        if previous is None and not self.ignore_option_errors:
            msg = (
                ...
                "Some dynamic fields need to be specified via `tools.setuptools.dynamic`"
    ...
    def _obtain(self, dist: "Distribution", field: str, package_dir: Mapping[str, str]):
        if field in self.dynamic_cfg:
            return self._expand_directive(
                f"tools.setuptools.dynamic.{field}",
        ...
        if "optional-dependencies" in self.dynamic_cfg:
            ...
            return {
                group: _parse_requirements_list(self._expand_directive(
                    f"tools.setuptools.dynamic.optional-dependencies.{group}",
                    directive,
                    {},
                ))
...
class _InvalidFile(UserWarning):
    ...
    DO NOT include the `[project]` or `[tools.setuptools]` tables in your file.
    \n\n!!
    """
